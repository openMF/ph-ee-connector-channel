...
public class DmtOperationsRouteBuilder extends BaseErrorHandlerRoute {
    ...
    void configureRoutes() {
        // Add Sender info
        // todo do request handling and trigger the zeebe message task [sender_info] with correlationId as [transactionId]
        from(path.postPath("/senderinfo"))
                .id("post-sender-info")
                .log(LoggingLevel.INFO, "## CHANNEL -> Post sender info request: ${body}")
        ...
        from(path.postPath("/otp/verify"))
                .id("verify-otp")
                .log(LoggingLevel.INFO, "## CHANNEL -> Verify OTP request: ${body}")
                .setBody(constant("This API will trigger the verify OTP path: ${body}"));

        // Resend OTP
        // todo do request handling and trigger the zeebe message task [otp] with correlationId as [transactionId]
        from(path.postPath("/otp/resend"))
                .id("resend-otp")
                .log(LoggingLevel.INFO, "## CHANNEL -> Resend OTP request: ${body}")
                .setBody(constant("This API will trigger the resend OTP path: ${body}"));

        // Beneficiary lookup
        // todo do request handling and trigger the zeebe message task [beneficiary_details] with correlationId as [transactionId]
        from(path.postPath("/beneficiarydetails"))
                .id("post-beneficiary-details")
