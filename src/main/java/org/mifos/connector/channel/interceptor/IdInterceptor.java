    ...
    private boolean checkIdempotency(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(correlationId.isBlank()) {
            ...
            errorDTO = new PhErrorDTO.PhErrorDTOBuilder(PaymentHubError.MandatoryValueNotSupplied)
                    ...
                    .developerMessage("Pass the valid header value for " + CLIENTCORRELATIONID)
                    .build();
            createError(response, errorDTO);
            return false;
        }
        String key = createRedisKey(request.getHeader("Platform-TenantId"),
                correlationId,request.getRequestURI(),keyFormat);
        logger.debug("X-CorrelationID value is {}", correlationId);
            if (Boolean.TRUE.equals(redisTemplate.hasKey(key))) {
                ...
                redisTemplate.opsForValue().set(key, correlationId, cacheRetencyDuration,
                        TimeUnit.DAYS);
                return true;
            }
