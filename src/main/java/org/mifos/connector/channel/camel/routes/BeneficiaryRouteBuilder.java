...
public class BeneficiaryRouteBuilder extends BaseErrorHandlerRoute {
    ...
    void configureRoutes() {
        ...
        from(path.getPath("")).id("get-all-beneficiary").log(LoggingLevel.INFO, "## CHANNEL -> Get all beneficiaries request: ${body}")
                .setBody(constant("This API will fetch all the beneficiaries"));

        // Get Beneficiary
        // todo implement the redirection to the dmt-connector
        from(path.getPath("{beneficiaryId}")).id("get-beneficiary")
        ...
        // Add Beneficiary
        // todo implement the redirection to the dmt-connector
        from(path.postPath("")).id("add-beneficiary").log(LoggingLevel.INFO, "## CHANNEL -> Add beneficiary request: ${body}")
                .setBody(constant("This API will add the beneficiary: ${body}"));

        // Delete Beneficiary
        // todo implement the redirection to the dmt-connector
        from(path.deletePath("{beneficiaryId}")).id("delete-beneficiary")
